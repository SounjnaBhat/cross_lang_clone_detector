...
export default function Sidebar() {
  ...
  const menuItems = [
    ...
    { name: 'LLVM Pipeline', path: '/pipeline', icon: GitFork },
    { name: 'IR Explorer', path: '/ir-explorer', icon: Code },
    { name: 'CFG Viewer', path: '/cfg-viewer', icon: GitFork },
    { name: 'DFG Viewer', path: '/dfg-viewer', icon: GitFork },
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`relative flex items-center gap-3.5 px-4 py-3 rounded-xl text-sm font-medium transition-all group ${isActive
                  ? 'text-white'
              ...
              >
                ...
                <Icon
                  size={18}
                  className={`relative z-10 transition-transform group-hover:scale-110 ${isActive ? 'text-purple-400' : 'text-zinc-400 group-hover:text-zinc-200'
                    }`}
                />
